    ...
    return(
      <div style={{
        ...
        <div className={'flex-space-between-column-center'} style={{ padding: '50px 0px' }}>
          ...
          <span className={'commonBoldFont'} style={{ fontSize: 16, color: '#000',paddingTop: 10 }} >支付成功</span>
          <span className={'commonBoldFont'} style={{ fontSize: 16, color: '#000',paddingTop: 10 }} >已付款：
            <span className={'commonBoldFont'} style={{ fontSize: 18, color: '#000' }} >45.0</span>元
          </span>
        </div>
        <div style={{ padding: 20 }}>
          <button className={'completeBtn'} onClick={() => history().goBack()}>完成</button>
        </div>
      </div>
    )
  }
  ...
  public render () {
    return (
      <div style={{
        ...
        <Head title={'支付结果'} titleColor={'#000000'} showLeftIcon={true} backgroundColor={'#ffffff'} leftIconColor={'grey'} showLine={true}/>
        {this.renderContent()}
      </div>
    )
  }
